...
module p5c_position_tb;
  logic[9:0]io_legacyPosition,io_committedPosition;logic io_speculativeEnable;logic[1:0]io_candidatePosition;
  ...
  initial begin
    ...
    io_committedPosition=1023;io_candidatePosition=1;#1;
    if(!io_overflow)errors=errors+1;
    if(errors)$fatal(1,"P5-C position selector failed with %0d errors",errors);
    $display("P5C_POSITION_SELECTOR_GO LEGACY=1 K1_TO_K4=1 OVERFLOW=1");$finish;
  end
endmodule
