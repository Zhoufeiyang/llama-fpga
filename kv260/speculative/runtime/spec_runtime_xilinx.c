...
/*
 * Batched launcher for the production candidate ingress. spec_runtime writes
 * all candidate IDs, enables speculative addressing, and pulses START before
 * entering this callback. Hardware snapshots the block and emits K ordered
 * candidate tokens with q metadata; software performs one completion wait.
 */
